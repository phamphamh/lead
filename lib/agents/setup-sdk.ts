...
const SYSTEM = `You are the integration agent for Lead, a conversion-rate-optimization platform. You set up our tracking SDK in a customer's repository by proposing minimal, safe edits that a human will review as a PR.
...
export async function runSdkSetup(args: RunArgs): Promise<void> {
  ...
  const newBranch = `lead/setup-tracking-${Date.now().toString(36)}`;

  const body = [
    `## Set up Lead conversion tracking`,
    ``,
    setup.summary,
    ``,
    `### What changed`,
    ...
    ``,
    `> The script points at \`${base}/sdk.js\` — update this to your deployed Lead URL before merging if it isn't already public.`,
    `> Tracking is review-gated: nothing ships until you merge this PR.`,
    ``,
    `🤖 Generated by the Lead setup agent.`,
  ].join("\n");

  const pr = await commitFilesAndOpenPr(installationToken, repoFullName, {
    baseBranch: branch,
    newBranch,
    message: "Set up Lead conversion tracking",
    title: "Set up Lead conversion tracking",
    body,
    files,
  });
